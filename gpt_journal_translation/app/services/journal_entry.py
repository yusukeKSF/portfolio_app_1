...
from collections import defaultdict # スプレッドシート入力時の重複した科目について合算と相殺して表示
...
また、「生産高比例法」が選択された場合は、当期の生産量と総生産可能量を含めてください。生産量とは、「使用量」「稼働量」なども含めてください。
...
以下の文書は取引明細をOCRで読み取ったものです。
この文から、会計仕訳として必要な情報を構造化してJSONで出力してください。

特に、減価償却方法が「生産高比例法」の場合には、以下の情報を必ず含めてください：


- "method": 償却方法（例："生産高比例法"）
- "current_volume": 当期の生産量（整数で）
- "total_volume": 総生産可能量（整数で）
- "amount": 資産の取得価格（整数で）
- "asset_name": 減価償却対象となる固定資産名（例："機械", "車両運搬具" など）
- "summary": 固定資産名 + の減価償却（例："機械の減価償却"）

可能であれば仕訳は entries[] に分け、debit/credit に分けてください。
...
{{
  ...
  "total_volume": 10000,
  "entries": [
    {{ "debit": "減価償却費", "credit": "減価償却累計額", "amount": 30000 }}
  ]
}}
...
# 減価償却累計額を資産ごとに管理
def extract_asset_name(summary: str) -> str:
    """
    例: "備品の減価償却" → "備品"
        "車両運搬具の減価償却費" → "車両運搬具"
    """
    ...
    match = re.match(r"(.+?)の減価償却", summary)
    return match.group(1) if match else summary  # fallback to full string if no match
...
def convert_gpt_entries_to_transaction(entry_data: dict) -> dict:
    ...
    def extract_asset_name(summary: str) -> str:
        import re
        # match = re.match(r"(.+?)の減価償却", summary or "")
        ...
        if not summary:
            return ""
        # パターン1: 「〇〇の減価償却」 or 「〇〇の減価償却費」
        m1 = re.match(r"(.+?)の減価償却(費)?", summary)
        if m1:
            return m1.group(1)

        # パターン2: 「〇〇減価償却」または「〇〇減価償却費」
        m2 = re.match(r"(.+?)減価償却(費)?", summary)
        if m2:
            return m2.group(1)
    ...
    is_depreciation = entry_data.get("type") == "depreciation"
    summary = entry_data.get("summary", "").strip()
    
    # ✅ まず asset_name フィールドを優先
    asset_name = entry_data.get("asset_name", "").strip()
    
    if is_depreciation and not asset_name:
        asset_name = extract_asset_name(summary)
    
    if is_depreciation and not asset_name:
        print(f"⚠️ 減価償却の資産名が summary（「{summary}」）から抽出できませんでした。")
        print("➡️ 勘定科目は『減価償却累計額』のままで処理されます。")

    for e in entry_data.get("entries", []):
        debit_summary[e["debit"]] += int(e["amount"])

        credit_account = e["credit"]
        if is_depreciation and credit_account == "減価償却累計額" and asset_name:
            credit_account = f"{asset_name}減価償却累計額"

        credit_summary[credit_account] += int(e["amount"])
